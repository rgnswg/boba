...
void AddCubeToMesh(Vector3 pos, Vector3 size, int* vIndex, float* vertices, float* texcoords, float* normals) {
    float x = pos.x;
    float y = pos.y;
    float z = pos.z;
    float w = size.x / 2.0f;
    ...
    // 6 caras * 2 triangulos * 3 vertices = 36 vertices por cubo (sin indices para simplificar generación)
    
    // Normales
    Vector3 nFront = {0,0,1}; Vector3 nBack = {0,0,-1};
    Vector3 nTop = {0,1,0};   Vector3 nBottom = {0,-1,0};
    Vector3 nRight = {1,0,0}; Vector3 nLeft = {-1,0,0};

    // Helper macro para añadir vertice
    #define PUSH_VERT(px, py, pz, nx, ny, nz, tx, ty) \
        vertices[*vIndex*3+0]=px; vertices[*vIndex*3+1]=py; vertices[*vIndex*3+2]=pz; \
        normals[*vIndex*3+0]=nx;  normals[*vIndex*3+1]=ny;  normals[*vIndex*3+2]=nz; \
        texcoords[*vIndex*2+0]=tx; texcoords[*vIndex*2+1]=ty; \
        (*vIndex)++;

    // Front Face
    PUSH_VERT(x-w, y-h, z+d, 0,0,1, 0,1); PUSH_VERT(x+w, y-h, z+d, 0,0,1, 1,1); PUSH_VERT(x-w, y+h, z+d, 0,0,1, 0,0);
    PUSH_VERT(x+w, y+h, z+d, 0,0,1, 1,0); PUSH_VERT(x-w, y+h, z+d, 0,0,1, 0,0); PUSH_VERT(x+w, y-h, z+d, 0,0,1, 1,1);

    // Back Face
    PUSH_VERT(x-w, y+h, z-d, 0,0,-1, 1,0); PUSH_VERT(x+w, y-h, z-d, 0,0,-1, 0,1); PUSH_VERT(x-w, y-h, z-d, 0,0,-1, 1,1);
    PUSH_VERT(x+w, y-h, z-d, 0,0,-1, 0,1); PUSH_VERT(x-w, y+h, z-d, 0,0,-1, 1,0); PUSH_VERT(x+w, y+h, z-d, 0,0,-1, 0,0);

    // Top Face
    PUSH_VERT(x-w, y+h, z-d, 0,1,0, 0,1); PUSH_VERT(x-w, y+h, z+d, 0,1,0, 0,0); PUSH_VERT(x+w, y+h, z+d, 0,1,0, 1,0);
    PUSH_VERT(x+w, y+h, z+d, 0,1,0, 1,0); PUSH_VERT(x+w, y+h, z-d, 0,1,0, 1,1); PUSH_VERT(x-w, y+h, z-d, 0,1,0, 0,1);

    // Bottom Face
    PUSH_VERT(x-w, y-h, z-d, 0,-1,0, 1,1); PUSH_VERT(x+w, y-h, z-d, 0,-1,0, 0,1); PUSH_VERT(x-w, y-h, z+d, 0,-1,0, 1,0);
    PUSH_VERT(x+w, y-h, z+d, 0,-1,0, 0,0); PUSH_VERT(x-w, y-h, z+d, 0,-1,0, 1,0); PUSH_VERT(x+w, y-h, z-d, 0,-1,0, 0,1);

    // Right Face
    PUSH_VERT(x+w, y-h, z-d, 1,0,0, 1,1); PUSH_VERT(x+w, y+h, z-d, 1,0,0, 1,0); PUSH_VERT(x+w, y+h, z+d, 1,0,0, 0,0);
    PUSH_VERT(x+w, y+h, z+d, 1,0,0, 0,0); PUSH_VERT(x+w, y-h, z+d, 1,0,0, 0,1); PUSH_VERT(x+w, y-h, z-d, 1,0,0, 1,1);

    // Left Face
    PUSH_VERT(x-w, y-h, z-d, -1,0,0, 0,1); PUSH_VERT(x-w, y-h, z+d, -1,0,0, 1,1); PUSH_VERT(x-w, y+h, z+d, -1,0,0, 1,0);
    PUSH_VERT(x-w, y+h, z+d, -1,0,0, 1,0); PUSH_VERT(x-w, y+h, z-d, -1,0,0, 0,0); PUSH_VERT(x-w, y-h, z-d, -1,0,0, 0,1);
}
...
void Map_Init(GameMap* map) {
    ...
    Mesh mesh = { 0 };
    mesh.vertexCount = wallCount * 36; // 36 vertices por cubo (6 caras * 2 tris * 3 verts)
    ...
    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            if (!map->walkable[y * map->width + x]) {
                ...
                AddCubeToMesh(pos, cubeSize, &vIndex, mesh.vertices, mesh.texcoords, mesh.normals);
            }
        }
    }
    ...
    ImageDrawRectangleLines(&blockImg, (Rectangle){0,0,64,64}, 4, BLACK); // Borde mas grueso
...
void Map_Unload(GameMap* map) {
    if (map->walkable) free(map->walkable);
    
    // UnloadModel libera el Mesh asociado tambien
    // PERO tenemos que liberar la textura manualmente porque la asignamos nosotros
    UnloadTexture(map->mapModel.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture);
