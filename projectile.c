...
void Proj_Spawn(ProjectileManager *pm, Vector3 pos, Vector3 dir, float speed,
                float life, float radius, float damage, Team team,
                Color color) {
  for (int i = 0; i < MAX_PROJECTILES; i++) {
    if (!pm->pool[i].active) {
...
void Proj_Update(ProjectileManager *pm, float dt, Entity *targets[],
                 int targetCount) {
  for (int i = 0; i < MAX_PROJECTILES; i++) {
    if (!pm->pool[i].active)
      continue;
    ...
    for (int t = 0; t < targetCount; t++) {
      ...
      // Excepción: TEAM_NEUTRAL recibe daño de todos (y sus proyectiles dañan a
      // todos)
      if (p->team != TEAM_NEUTRAL && ent->team == p->team)
        continue;

      if (CheckCollisionSpheres(nextPos, p->radius, ent->position,
                                ent->radius)) {
        // IMPACTO!
