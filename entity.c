...
void Entity_Init(Entity *ent) {
  ent->netId = -1; // Sin asignación de red por defecto
...
void Entity_Update(Entity *ent, float dt) {
  ...
  if (ent->isDashing) {
    ent->position =
        Vector3Add(ent->position, Vector3Scale(ent->dashVelocity, dt));
    ent->dashTimer -= dt;
    if (ent->dashTimer <= 0) {
      ent->isDashing = false;
      ent->dashTimer = 0;
    }
    return; // Si estamos dasheando, no hacemos nada más (cooldowns siguen
            // bajando?)
